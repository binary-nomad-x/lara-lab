...
* **Volume Syncing (`-v`)**: The `.:/var/www/html` mapping ensures that any code you save in VS Code on Windows is
  instantly updated inside the Linux container.
* **The `.env` Factor**: Always ensure your `DB_HOST` in `.env` matches your service name in `docker-compose.yml` (in
  your case, `DB_HOST=db`).
